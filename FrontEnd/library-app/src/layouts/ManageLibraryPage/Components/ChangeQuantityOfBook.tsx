import { useEffect, useRef, useState } from "react";
import BookModel from "../../../models/BookModel";
import { useOktaAuth } from "@okta/okta-react";
import { useDebounce } from "../../utils/hooks";


export const ChangeQuantityOfBook: React.FC<{book:BookModel, deleteButton: any}> = (props,key) =>{
    const [isLoading, setIsLoadng] = useState(true);
    const [httpError, setHttpError] = useState(null);
    const {authState} = useOktaAuth();    
    const[quantity,setQuantity] = useState<number>(0);
    const [remaining,setRemaining] = useState<number>(0);
    const [updatedQuantity, setUpdatedQuantity] = useState(0);
    const debouncedQuantity = useDebounce(updatedQuantity);
    const isMounted = useRef(false);
    
    useEffect(() => {
        const fetchBookInState = () =>{
            props.book.copies ? setQuantity(Number(props.book.copies)) : setQuantity(0);
            props.book.copiesAvailable ? setRemaining(props.book.copiesAvailable) : setRemaining(0); 
        };
        fetchBookInState();
    },[]);

    
     useEffect(()=>{
        if (isMounted.current) {
        console.log("updated :" ,updatedQuantity);
        const changeQuantity = async() =>{
            const url=`http://localhost:8080/api/admin/secure/change/quantity/${props.book.id}?quantity=${updatedQuantity}`;
            const requestOptions = {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
                    "Content-Type": "application/json"
                }
            };
            const changeQuantityResponse = await fetch(url,requestOptions);
            if(!changeQuantityResponse.ok){
                throw new Error("Something went wrong!");
            }
        };
        changeQuantity().catch((error: any) => {
            setIsLoadng(false);
            setHttpError(error.message);
          });}
        else{
            isMounted.current = true;
        }
    },[debouncedQuantity])

    function increaseQuantity(){
        setUpdatedQuantity(quantity+1);
        setQuantity(quantity+1); 
        setRemaining(remaining+1);
        console.log("increase",quantity)
     }

    
    function decreaseQuantity(){
        setUpdatedQuantity(quantity-1);
        setQuantity(quantity-1); 
        setRemaining(remaining-1)
        console.log("decrease",quantity)
     }

    async function deleteBook(){
        const url=`http://localhost:8080/api/admin/secure/delete/book?bookId=${props.book.id}`;
            const requestOptions = {
                method: "DELETE",
                headers: {
                    Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
                    "Content-Type": "application/json"
                }
            };
    
            const deleteBookResponse = await fetch(url,requestOptions);
            if(!deleteBookResponse.ok){
                throw new Error("Something went wrong!");
            }

            props.deleteButton();


    }




    return(
    <div className="card mt-3 shadow p-3 mb-3 bg-body rounded">
        <div className="row g-0">
            <div className="col-md-2">
                <div className="d-none d-lg-block">
                    {props.book.img ? 
                    <img src = {props.book.img} width="123" height="196" alt="book" />
                    :
                    <img src = {require('../../../Images/BooksImages/book-luv2code-1000.png')}
                     width="123" height="196" alt="book" />
                    }
                </div>
                <div className="d-lg-none d-flex justify-content-center align-items-center">
                    {props.book.img ? 
                    <img src = {props.book.img} width="123" height="196" alt="book" />
                    :
                    <img src = {require('../../../Images/BooksImages/book-luv2code-1000.png')}
                     width="123" height="196" alt="book" />
                    }
                </div>
            </div>

            <div className="col-md-6">
                <div className="card-body">
                    <h5 className="card-title">{props.book.author}</h5>
                    <h4>{props.book.title}</h4>
                    <p className="card-text"> {props.book.description} </p>                   
                </div>
            </div>

            <div className="mt-3 col-md-4">
                <div className="d-flex justify-content-center align-items-center">
                <p className="card-text">Total Quantity: <b>{quantity}</b></p>
                </div>

                <div className="d-flex justify-content-center align-items-center">
                    <p>Books Remaining: <b>{remaining}</b></p>
                </div>
            </div>
            <div className="mt-3 col-md-1">
                <div className="d-flex justify-content-start">
                    <button className="m-1 btn btn-md btn-danger" onClick={deleteBook}>Delete</button>
                </div>
            </div>
            
            <button className="m-1 btn btn-md main-color text-white" onClick={increaseQuantity}>Add Quantity</button>
            <button className="m-1 btn btn-md btn-warning" onClick={decreaseQuantity}>Decrease Quantity</button>

        </div>
    </div>
    );
}
export default ChangeQuantityOfBook;